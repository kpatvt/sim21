from sim21.old.thermo.ThermoAdmin import CustomCommandObject
from sim21.old.solver.Error import SimError
from sim21.old.unitop.XYTable import ATable
...
class LightEnds(object):
    ...
    def CleanUp(self):
        self.parentObj = None
        for key in list(self.lightEndDict.keys()):
    ...
    def DeleteObject(self, obj):
        if obj.name in self.lightEndDict:
            del self.lightEndDict[obj.name]
            self.IsUpToDate(0)
        obj.CleanUp()

    def GetObject(self, name):
        if name in self.lightEndDict:
...
class BasicOilObject(object):
    ...
    def CleanUp(self):
        ...
        if self.childDict:
            for key in list(self.childDict.keys()):
    ...
    def GetContents(self):
        ...
        if self.childDict:
            children = list(self.childDict.keys())
            children.sort()
            for key in children:
    ...
    def SetChildDictType(self, type_of):
        self.childType = type_of
        self.childDict = OilDict(type_of)
    ...
    def AddObject(self, obj, name):
        ...
        if isinstance(obj, OilParameter):
            ...
            # rename object
            if obj not in self.parentObj.childDict:
    ...
    def DeleteObject(self, obj):
        if isinstance(obj, OilParameter):
            if obj.name in self.parameters:
                del self.parameters[obj.name]
                self.isUpTodate = 0

        elif isinstance(obj, self.childType):
            if obj.name in self.childDict:
                thCase = self.GetThermoCase()
                if thCase:
...
class Assay(BasicOilObject):
    ...
    def AttachToPort(self, port):
        if port not in self._attachedPort:
