...
from sim21.old.thermo.Hypo import GetSimHypoStrings, GetSimHypoDoubles, GetCompoundPropertyLists
...
class ThermoAdmin(object):
    ...
    def SetNewThermoProvider(self, modName, className, args=()):
        ...
        notSupp = list(filter(myFilter, reqProps))
        if len(notSupp) > 0:
    ...
    def __ImportModule(self, modName):
        ...
        return import_module('sim21.old.thermo.{0}'.format(Sim21ThermoModName))
    ...
    def SupportsProvider(self, provider):
        ...
        """
        return provider in self.thDict
    ...
    def ChangeThermoCaseName(self, provider, oldThName, newThName):
        ...
        if (newThName in avThCases) or (oldThName not in avThCases):
            return
    ...
    def DeleteThermoCase(self, provider, thName, cleaningUp=0):
        ...
        if thName not in self.GetAvThCaseNames(provider):
    ...
    def CheckThermoVersion(self):
        for provider in list(self.thDict.keys()):
    ...
    def GetArrayPropertyNames(self, provider):
    ...
    def SetCommonPropertyNames(self, provider, propList):
        ...
        if ZFACTOR_VAR not in propList:
    ...
    def GetArrayProperty(self, provider, thName, prop1, prop2, phase, frac, prop):
        """
        ...
        """
        return self.thDict[provider].GetArrayProperty(thName, prop1, prop2, phase, frac, prop)
    ...
    def UpdateSaveInfo(self):
        ...
        for th in list(self.thDict.items()):
    ...
    def MergeThermoAdmin(self, fThAdmin):
        ...
        lThAdmin = self
        lProvNames = list(lThAdmin.thDict.keys())
        lProv = list(lThAdmin.thDict.values())
        ...
        for fProvName, fProv in list(fThAdmin.thDict.items()):
...
class ThermoCase(object):
    ...
    def LinkUnitOp(self, unitop):
        ...
        """
        if unitop not in self.linkedUnitOps:
    ...
    def GetLinkedUnitOpPaths(self):
        ...
        return [uo.GetPath() for uo in self.linkedUnitOps]
    ...
    def GetKijPaneValues(self, ipMatrName, paneIdx):
        ...
        if (ipMatrName, paneIdx) in self.storedKijVals:
...
class EnvelopeResults(object):
    def __init__(self, retCode, retMessage, pointCount, types, pVals, tVals, kVals, type_of='PT'):
    ...
    def SetCritical(self):
        ...
        if self.type == 'PT':
            for i in range(self.pointCount):
                if self.pointTypes[i] == 2 and self.criticalPoint[0] is None:
                    self.criticalPoint = (self.pValues[i], self.tValues[i])
                elif self.pointTypes[i] == 3 and self.cricondenbar[0] is None:
                    self.cricondenbar = (self.pValues[i], self.tValues[i])
                elif self.pointTypes[i] == 4 and self.cricondentherm[0] is None:
...
class IPMatrices(object):
    ...
    def __init__(self, propPkgObj):
        ...
        self.cmpNamesFixed = list(map(SubSpaceForUnder, self.cmpNames))
...
class IPMatrix(object):
    ...
    def GetAllIPInfoForCmp(self, cmpName1):
        ...
        return list(map(self.GetIPInfoActiveCmp, self.cmpNames))
    ...
    def GetIPInfo(self):
        ...
        ipInfo = list(map(self.GetAllIPInfoForCmp, self.cmpNames))
        del self.thCase
        del self.cmpNames
...
class IPPane(object):
    ...
    def GetValues(self):
        ...
        if self.name != IPINFO:
            # return self.ipVals
            return list(map(list, self.ipVals))
        else:
            return self.ipVals
...
class IPValues(object):
    def __init__(self, ipPane, name):
        ...
        if ipPane.name == IPINFO:
            ...
            self.vals = np.array(ipPane.ipVals[cmpIdx, :], dtype=float)
...
class IPValue(object):
    ...
    def SetValue(self, value, status=None):
        ...
        if cmpName1 not in cmpNames:
            cmpName1 = re.sub('_', ' ', cmpName1)
        if cmpName2 not in cmpNames:
...
from sim21.old.thermo import Oils
