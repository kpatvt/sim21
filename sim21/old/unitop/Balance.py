...
class Balance:
    ...
    def _AddInputByType(self, port):
        ...
        if isinstance(port, Ports.Port_Energy):
            ...
            raise SimError("BalanceInvalidPort")
    ...
    def DoMassBalance(self, calcStatus=CALCULATED_V):
        ...
        if len(self._matIn):
            aPort = self._matIn[0]
        elif len(self._matOut):
            aPort = self._matOut[0]
        else:
            balanced = 1
            return balanced
    ...
    def DoMoleBalance(self, canUseEnth, calcStatus):
        ...
        if totPorts == 2:
            ...
            if canUseEnth:
                ...
                for eneP in self._eneIn + self._eneOut:
                    ...
                    if eneFlow is None or eneFlow != 0.0:
                        allEneAreZero = 0
                        break
                if allEneAreZero:
                    for i in range(len(myPortLst) - 1):
        ...
        if nuMissing == 0:
            ...
            try:
                if abs(np.linalg.det(np.array(a))) < 0.0001:
                    return balanced

                flows = np.linalg.solve(np.array(a), np.array(b))
            except:
                return balanced
    ...
    def DoVolBalance(self, calcStatus=CALCULATED_V):
        ...
        if vPort is None or xPort is None:
            balanced = 1
            return balanced
        ...
        for port in self._matIn + self._matOut:
            ...
            if port is xPort:
                xIdx = cnt
            cnt += 1
        if vIdx is None or xIdx is None:
            return balanced
        ...
        if mode == VOLFLOW_VAR:
            ...
            if vH is None:
                ...
                enthalpies = np.array(enthalpies, dtype=float)
        ...
        while not converged and iter < maxIter:
            ...
            if mode == STDVOLFLOW_VAR:
                ...
                if xT is not None:
                    ...
                    xH = np.sum(moleFlows * signs * enthalpies) / xMoleFlow
                    if xSign > 0.0:
                        xH *= -1.0
