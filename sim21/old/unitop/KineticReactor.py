...
from sim21.old.solver import setup
from sim21.old.solver.Messages import MessageHandler
from sim21.old.solver.Variables import *
from sim21.old.unitop import PipeSegment  # Import this just to have the same constants
from sim21.old.unitop import UnitOperations, BaseForReactors
# Generic reactor constants
from sim21.old.unitop.BaseForReactors import REACTION, NURXN_PAR, RXNORDER_PAR
from sim21.old.unitop.BaseForReactors import ReactorObject
...
_mathFuncs = [x for x in list(math.__dict__.keys()) if '_' not in x]
_operators = ['=', '==', '+', '-', '*', '/', '\\', '**', '^']
    ...
    if nuTokens == 2:
        ...
        if splitPath[0] == IN_PORT:
            ...
            # Extensive variables
                ...
                flows = glbX[glbUnitOp._1stFCmp0Idx:glbUnitOp._1stFCmp0Idx + nuCmps]
                massFlow = np.sum(flows * glbUnitOp._pureCmpMW)
                return massFlow
            elif prop == VOLFLOW_VAR:
                P, T = glbX[glbUnitOp._T0Idx], glbUnitOp._P0
                flows = glbX[glbUnitOp._1stFCmp0Idx:glbUnitOp._1stFCmp0Idx + nuCmps]
                molarFlow = np.sum(flows)
    ...
    # Likely a property of a specific compound
        ...
        if splitPath[0] == IN_PORT and cmpIdx >= 0:
            ...
            if prop == FRAC_VAR:
                ...
                massFracs = massFlows / np.sum(massFlows)
...
"""
    ...
    def SetParameterValue(self, paramName, value):
        ...
        # delete parameter if order = 0
            ...
            if value == 0 and paramName in self.parameters:
    ...
    def DeleteCompound(self, cmpName):
        ...
        if FWDORDER_PAR + cmp in self.parameters:
            del self.parameters[FWDORDER_PAR + cmp]
        if REVORDER_PAR + cmp in self.parameters:
    ...
    def ValidateParameter(self, paramName, value):
        ...
        if paramName == RXN_RATE_EQ_PAR:
            ...
            cmpNames = [re.sub(' ', '_', cmpName) for cmpName in cmpNames]
                ...
                if not line or line[0] == '#':
                    ...
                    for i in range(nuTokens):
                        ...
                        if token in _reservedWords:
                            ...
                            return False
                        if token not in allValidStrings:
...
class CSTR(EquationSolver.EquationBasedOp):
    ...
    def ValidateParameter(self, paramName, value):
        ...
        if paramName == RXNPHASE_PAR:
            if value not in (LIQUID_PHASE, VAPOUR_PHASE, OVERALL_PHASE, None):
                return False
        return True
    ...
    def PrepareForSolve(self):
        ...
        if ready:
            self._pureCmpMW = np.zeros(self._nuCmps, dtype=float)
    ...
    def GetStoichCoeffsArray(self):
        ...
        # Dimension variables
        coeffsArr = np.zeros((nuCmps, nuRxn), dtype=float)
    ...
    def LoadUnknowns(self, u):
        ...
        missingInlet = False
        if F0 is not None and z0 is not None:
            FCmp0 = F0 * np.array(z0, dtype=float)
            if F0:
        ...
        if missingInlet:
            FCmp0 = np.array(FCmp, dtype=float)
        elif missingOutlet:
            FCmp = np.array(FCmp0, dtype=float)
        ...
        for i in range(self._nuCmps):
            ...
            if not missingOutlet and not missingInlet:
                ...
                if not usedSpec and 0 not in self._stoichCoeffsArray[i, :]:
                    usedSpec = True
                else:
                    isSpec = False
    ...
    def CalculateRHS(self, x, rhs, isFix, initx, eqnNo=0):
        ...
        rateRxn = np.zeros(nuRxn, dtype=float)
        P0 = self._P0
        P = self._P
        ...
        rhs[eqnNo:eqnNo + nuCmps] = (FCmp0 - FCmp + (
            V * np.matmul(stoichCoeffsArray, rateRxn)) * 3600.0) / self.scaleFactorF
        eqnNo = eqnNo + nuCmps
...
class PFR(EquationSolver.EquationBasedOp):
    ...
    def __setstate__(self, oldState):
        ...
        if 'integrator' in self.__dict__:
            if self.integrator:
                try:
    ...
    def CreateDeltaPCalcPorts(self):
        ...
        if not self.GetPort(PipeSegment.ROUGH_PORT):
        ...
        if not self.GetPort(PipeSegment.ELEVATION_PORT + '0'):
        ...
        if not self.GetPort(PipeSegment.ELEVATION_PORT + '1'):
    ...
    def ParameterChanged(self, paramName, value):
        ...
        if paramName == NUSECTIONS_PAR:
            ...
            # Dimension variables
            self.P = np.zeros(nuSections + 1, dtype=float)
            self.H = np.zeros(nuSections + 1, dtype=float)
            self.Q = np.zeros(nuSections, dtype=float)
            self.U = np.zeros(nuSections, dtype=float)

            self._PIdx = np.zeros(nuSections + 1, dtype=int)
            self._HIdx = np.zeros(nuSections + 1, dtype=int)
            self._QIdx = np.zeros(nuSections, dtype=int)
            self._UIdx = np.zeros(nuSections, dtype=int)
    ...
    def Solve(self):
        ...
        if converged:
            ...
            if self.eneMode == USES_Q and self.tAmb:
                avgVal = np.sum(self.U) / len(self.U)
        ...
        if mf0 is not None and self._z0 is not None and None not in self._z0:
            self._z0 = np.array(self._z0, dtype=float)
            self._fCmp0 = mf0 * self._z0
        mf = outPort.GetPropValue(MOLEFLOW_VAR)
        if mf is not None and self._z is not None and None not in self._z:
            self._z = np.array(self._z, dtype=float)
    ...
    def DimensionArrays(self, length):
        ...
        if self.F is None:
            self.F = np.zeros(length, dtype=float)
        else:
            oldLen = len(self.F)
            if oldLen >= length:
                self.F = self.F[:length]
            else:
                temp = np.array(self.F, dtype=float)
                self.F = np.zeros(length, dtype=float)
                self.F[:oldLen] = temp

        if self.T is None:
            self.T = np.zeros(length, dtype=float)
        else:
            oldLen = len(self.T)
            if oldLen >= length:
                self.T = self.T[:length]
            else:
                temp = np.array(self.T, dtype=float)
                self.T = np.zeros(length, dtype=float)
                self.T[:oldLen] = temp

        if self.P is None:
            self.P = np.zeros(length, dtype=float)
        else:
            oldLen = len(self.P)
            if oldLen >= length:
                self.P = self.P[:length]
            else:
                temp = np.array(self.P, dtype=float)
                self.P = np.zeros(length, dtype=float)
                self.P[:oldLen] = temp

        if self.Q is None:
            self.Q = np.zeros(length, dtype=float)
        else:
            oldLen = len(self.Q)
            if oldLen >= length:
                self.Q = self.Q[:length]
            else:
                temp = np.array(self.Q, dtype=float)
                self.Q = np.zeros(length, dtype=float)
                self.Q[:oldLen] = temp

        if self.U is None:
            self.U = np.zeros(length, dtype=float)
        else:
            oldLen = len(self.U)
            if oldLen >= length:
                self.U = self.U[:length]
            else:
                temp = np.array(self.U, dtype=float)
                self.U = np.zeros(length, dtype=float)
                self.U[:oldLen] = temp

        if self.H is None:
            self.H = np.zeros(length, dtype=float)
        else:
            oldLen = len(self.H)
            if oldLen >= length:
                self.H = self.H[:length]
            else:
                temp = np.array(self.H, dtype=float)
                self.H = np.zeros(length, dtype=float)
                self.H[:oldLen] = temp

        if self.x is None:
            self.x = np.zeros(length, dtype=float)
        else:
            oldLen = len(self.x)
            if oldLen >= length:
                self.x = self.x[:length]
            else:
                temp = np.array(self.x, dtype=float)
                self.x = np.zeros(length, dtype=float)
                self.x[:oldLen] = temp

        if self.r is None:
            self.r = np.zeros((length, self._nuRxn), dtype=float)
        else:
            oldLen = len(self.r)
            if oldLen >= length:
                self.r = self.r[:length, :]
            else:
                temp = np.array(self.r, dtype=float)
                self.r = np.zeros((length, self._nuRxn), dtype=float)
                self.r[:oldLen, :] = temp

        if self.f is None:
            self.f = np.zeros((length, self._nuCmps), dtype=float)
        else:
            oldLen = len(self.f)
            if oldLen >= length:
                self.f = self.f[:length, :]
        ...
        if self.C is None:
            self.C = np.zeros((length, self._nuCmps), dtype=float)
        else:
            oldLen = len(self.C)
            if oldLen >= length:
                self.C = self.C[:length, :]
            else:
                temp = np.array(self.C, dtype=float)
                self.C = np.zeros((length, self._nuCmps), dtype=float)
                self.C[:oldLen, :] = temp
    ...
    def GetInitValsForIntegrator(self):
        ...
        if self.u is not None and self.tAmb is not None:
            self.eneMode = USES_U
        elif self._totQ is not None:
            self.eneMode = USES_Q
        else:
        ...
        nuCmps = self._nuCmps
        otherVars = 2  # Q and P
        yInit = np.zeros(nuCmps + otherVars, dtype=float)
        yMin = -1.0E40 * np.ones(nuCmps + otherVars, dtype=float)
        yMax = 1.0E40 * np.ones(nuCmps + otherVars, dtype=float)
        yScale = np.ones(nuCmps + otherVars, dtype=float)

        # Quick way to find a max flow
        coeffs = self._stoichCoeffsArray
        zReactants = np.clip(coeffs, -1E100, 0.0)
        zProducts = np.clip(coeffs, 0.0, 1E100)
        zReactants = np.sum(zReactants)
        zProducts = np.sum(zProducts)
        tempVal1 = np.sum(zProducts)
        tempVal2 = -1.0 * np.sum(zReactants)
        tempVal = 1.0
        if isinstance(tempVal1, np.ndarray):
        ...
        # Flows
        yInit[:nuCmps] = np.array(self._fracs) * F
        yMin[:nuCmps] *= 0.0
        yMax[:nuCmps] = maxF
        yScale[:nuCmps] = max((1.0, max(np.abs(yInit[:nuCmps]))))
    ...
    def CalculateDerivatives(self, x, y, loadResults=False):
        ...
        rateRxn = np.zeros(nuRxn, dtype=float)
        hRxn = np.zeros(nuRxn, dtype=float)
        ...
        dF_dLen = self._area * np.matmul(stoichCoeffsArray, rateRxn) * 3600.0
        ...
        # qRxn = -crossArea * np.sum(hRxn*rateRxn) * 3600.0   #(m2) * (kJ/kmol) * (kmol/(s*m3) * (s/h) -> kJ/(m*h)
    ...
    def RoundValues(self, y, yMin, yMax, yScale):
        ...
        if min(f) < minAllowedFlow:
            ...
            valsToModif = list(map(operator.ge, np.abs(f), minAllowedFlow * np.ones(nuCmps, dtype=float)))

            # Round to 0.0
            f = np.where(valsToModif, f, 0.0)
    ...
    def StepToBoundaries(self, x, y, h):
        ...
        a = self._area
        deltaxVec = np.zeros(nuCmps, dtype=float)
        success = False
        mindeltax = None
        for j in range(nuCmps):
            if f[j] < 0.0:
                ...
                if deltax <= mindeltax:
                    ...
                    continue
                deltaf = deltax * a * np.matmul(stoich, r) * 3600.0
                newf = deltaf + lastf
                if min(newf) >= 0.0:
                    success = True
                    break
        while not success:
            ...
            deltaf = mindeltax * a * np.matmul(stoich, r) * 3600.0
            newf = deltaf + lastf
            if min(newf) >= 0.0:
                success = True
                break
    ...
    def SolveForVolume(self):
        ...
        length = self.lenPort.GetValue()
        vol = self.volPort.GetValue()
        if diam is not None and length is not None:
            self.volPort.SetValue(length * PI * (diam / 2.0) ** 2, CALCULATED_V)
        elif diam is not None and vol is not None:
            self.lenPort.SetValue(vol / (PI * (diam / 2.0) ** 2), CALCULATED_V)
        elif length is not None and vol is not None:
            self.diamPort.SetValue(2.0 * math.sqrt(vol / (length * PI)), CALCULATED_V)
    ...
    def PrepareForSolve(self):
        ...
        if not self._fracs or None in self._fracs:
            ...
            if not self._MW:
                ...
                self._MW = np.sum(np.array(MWLst, dtype=float) * np.array(self._fracs, dtype=float))
    ...
    def GetPropertyVecs(self, P, H, fracs, propList, phase):
        ...
        inProp1 = [P_VAR, P]
        inProp2 = [H_VAR, H]
        phase = phase * np.ones(len(P), dtype=int)
        fracs = fracs * np.ones((len(P), len(fracs)), dtype=float)
        vals = thAdmin.GetProperties(prov, case, inProp1, inProp2, phase, fracs, propList)
        return vals
