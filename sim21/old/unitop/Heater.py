...
from sim21.old.unitop import Balance, Tower, Custom
...
from sim21.old.unitop.PipeSegment import PressureDropModel
from sim21.old.support.kludges import cmp
...
class HeaterCooler(UnitOperations.UnitOperation):
    ...
    def GetObject(self, desc):
        ...
        if desc == P_VAR or desc == H_VAR:
            try:
                ...
                if nuSegs and varIn is not None and varOut is not None:
            ...
            try:
                q = qPort.GetValue()
                if q is not None and nuSegs:
                    qPerSeg = q / nuSegs
                    prof = np.ones(nuSegs + 1, dtype=float) * qPerSeg
            ...
            try:
                ...
                if qProf is not None:
                    prof = np.abs(np.add.accumulate(qProf))
                    self.storedProfiles[desc] = prof
                    return prof
                return None
            except:
    ...
    def Profile(self, propName, phase=OVERALL_PHASE):
        ...
        if profile is not None and len(profile) == nuSegments + 1:
            return profile
        ...
        if propName != VPFRAC_VAR and propName != MASSVPFRAC_VAR:
            if propName not in thAdmin.GetPropertyNames(prov):
                return None
        profile = None
        ...
        propNames = []
        if propName not in ignoreNames:
        ...
        if len(propNames) == 0:
            ...
            if mw is None or len(mw) != nuSegments + 1:
                # Get MW just in case
                propNames.append(MOLEWT_VAR)
            else:
        ...
        if isinstance(calcProfiles, str) or isinstance(calcProfiles, str):
            knownNames = list(storedProfiles.keys())
            for item in calcProfiles.split(' '):
                # get the name of the property
                item = item.split('_', 1)[-1]

                # add it to the list if necessary
                if (item not in ignoreNames) and (item not in propNames):
                    if (item not in knownNames) or (len(storedProfiles[item]) != nuSegments + 1):
                        propNames.append(item)

        # Dimension arrays
        nuProps = len(propNames)
        profilesBulk = np.zeros((nuProps, nuSegments + 1), dtype=float)
        profilesVap = np.zeros((nuProps, nuSegments + 1), dtype=float)
        profilesLiq = np.zeros((nuProps, nuSegments + 1), dtype=float)
        profilesVapFrac = np.zeros(nuSegments + 1, dtype=float)
        ...
        for nuProp in range(nuProps):
            storedProfiles[propNames[nuProp]] = np.array(profilesBulk[nuProp, :], dtype=float)
            storedProfiles['%s_%s' % (Tower.TOWER_VAP_PHASE, propNames[nuProp])] = np.array(profilesVap[nuProp, :],
                                                                                            dtype=float)
            storedProfiles['%s_%s' % (Tower.TOWER_LIQ_PHASE, propNames[nuProp])] = np.array(profilesLiq[nuProp, :],
                                                                                            dtype=float)

        # Store Vap fraction
        vf = storedProfiles[VPFRAC_VAR] = np.array(profilesVapFrac, dtype=float)
    ...
    def _CloneCreate(self):
        ...
        if self.GetPort(IN_PORT + 'Q') is not None:
    ...
    def Solve(self):
        ...
        # Solve if UA and a T in each side is known
                ...
                if nuUnkT == 1:
                    ...
                    if nuUnkF == 1:
                        ...
                        if converged:
                            foundT = x[idxUnkT]
                            if idxUnkT == 0:
                                self.hotPortIn.SetPropValue(T_VAR, foundT, CALCULATED_V | PARENT_V)
                            elif idxUnkT == 1:
                                self.hotPortOut.SetPropValue(T_VAR, foundT, CALCULATED_V | PARENT_V)
                            elif idxUnkT == 2:
                                if isCounterCurrent:
                                    foundT = x[idxUnkT + 1]
            ...
            while stay:
                ...
                if conv:
                    if TIsIn:
                        self.hotPortOut.SetPropValue(T_VAR, TIter, CALCULATED_V | PARENT_V)
                    else:
                        self.hotPortIn.SetPropValue(T_VAR, TIter, CALCULATED_V | PARENT_V)
                else:
    ...
    def SolveForQ(self, UA, hotInfo, coldInfo, PIter, pIter, scaleFactor):
        ...
        if self.lastLMTD is not None:
            ...
            try:
                ...
                if newLMTD is not None:
                    ...
                    HIter = H + S * Q * 3.6 / F  # Conversion from W to kJ/h
                    hIter = h + s * Q * 3.6 / f  # Conversion from W to kJ/h
                    TIter = thAdmin.GetProperties(Prov, Case, (P_VAR, PIter), (H_VAR, HIter), OVERALL_PHASE, Fracs,
                                                  [T_VAR])
                    tIter = thAdmin.GetProperties(prov, case, (P_VAR, pIter), (H_VAR, hIter), OVERALL_PHASE, fracs,
                                                  [T_VAR])
                    TIter = TIter[0]
        ...
        while iteration <= maxIter:
            ...
            if newton:
                ...
                if abs(newError) > abs(error):
                    if infoBisect is not None:
                        # Back to bisecting
                        (iteration, curr, try1, try2, newError, error, step) = infoBisect
                    else:
                        break
                else:
            ...
            # Far from solution. Do bisection
                ...
                if abs(newError) < switch and iteration > 3 and canSwitch == 1:
                    dx = curr - lastCurr
                    dF = newError - error
                    if abs(dx) > tinydt and abs(dF) > tinydt / scaleFactor:
                        ...
                        canSwitch = 0
                        dx_dF = dx / dF
                        infoBisect = (iteration, curr, try1, try2, newError, error, step)
...
class _Side(EquationSolver.EquationBasedOp):
    ...
    def UpdateStructure(self):
        ...
        if self._parCountCur is not None and parCountCur is not None:
    ...
    def GetArray(self, type_of):
        """Returns an array of values of a given type"""
        if type_of == T_VAR:
            return self.TArray
        elif type_of == ENERGY_VAR:
    ...
    def LoadUnknowns(self, u):
        ...
        HArray = np.zeros(nuSegments + 1, dtype=float)
        QArray = np.zeros(nuSegments, dtype=float)
        # self.TArray = TArray = np.zeros(nuSegments+1, dtype=float)
        # self.PArray = PArray = np.zeros(nuSegments+1, dtype=float)
        # self.HArray = HArray = np.zeros(nuSegments+1, dtype=float)
        # self.QArray = QArray = np.zeros(nuSegments, dtype=float)
        ...
        self.moleFlowIdx = None
        self.TIndex = np.zeros(nuSegments + 1, dtype=int)
        self.HIndex = np.zeros(nuSegments + 1, dtype=int)
        self.QIndex = np.zeros(nuSegments, dtype=int)
        ...
        isSpec = True
        if molarFlow is None:
            if initMode == SCRATCH_INIT or self.molarFlow is None:
    ...
    def CalculateRHS(self, x, rhs, isFix, initx, eqnNo=0):
        ...
        self.QArray[:] = np.take(x, self.QIndex)
        self.HArray[:] = np.take(x, self.HIndex)
        self.TArray[:] = np.take(x, self.TIndex)
    ...
    def GetObject(self, desc):
        ...
        if desc == COUNTER_CURRENT_PAR:
            ...
            return np.abs(np.add.accumulate(qOut))
        else:
            try:
                phase = OVERALL_PHASE
                propName = desc
    ...
    def Profile(self, propName, phase=OVERALL_PHASE):
        ...
        if isinstance(calcProfiles, str) or isinstance(calcProfiles, str):
            ...
            knownNames = list(self.storedProfiles.keys())
            for item in calcProfiles.split(' '):
                # get the name of the property
                item = item.split('_', 1)[-1]
...
class _HeatTransfer(EquationSolver.EquationBasedOp):
    ...
    def GetObject(self, desc):
        ...
        # return None
        # return np.add.accumulate(self.duty) / 3.6

        elif desc == 'lmtd':
            if hasattr(self, 'lmtd') and not self.lmtd or None in self.lmtd:
                return None
            return np.array(self.lmtd, dtype=float)

        elif desc == 'ua':
            if hasattr(self, 'ua') and not self.ua or None in self.ua:
                return None
            return np.array(self.ua, dtype=float) / 3.6
    ...
    def LoadUnknowns(self, u):
        ...
        # Dimension arrays
        duty = np.zeros(nuSegments, dtype=float)
        ua = np.zeros(nuSegments, dtype=float)
        lmtd = np.zeros(nuSegments, dtype=float)
        logTerm = np.zeros(nuSegments, dtype=float)
        usePinchEqn = np.zeros(nuSegments, dtype=int)
        countPinch = np.zeros(nuSegments, dtype=int)
        dutyIndex = np.zeros(nuSegments, dtype=int)
        uaIndex = np.zeros(nuSegments, dtype=int)
        ...
        if UA is None:
            isSpec = False
            if initMode == SCRATCH_INIT or self._UA is None:
                QArray = side1.GetArray(ENERGY_VAR)
                if side1.GetIsCoCurrent():
                    mySign = -1
                else:
                    mySign = 1
                Q = mySign * QArray[0]
    ...
    def CalculateRHS(self, x, rhs, isFix, initx, eqnNo=0):
        ...
        self.duty[:] = np.take(x, self.dutyIndex)
        if segType == ENERGY_VAR:
            self.ua[:] = np.take(x, self.uaIndex)
        else:
            self.ua[:] = np.ones(nuSegments, dtype=float) * UA
                        ...
                        for j in range(len(s1t) - 1):
                            ...
                            if s1t1 is None:
                                for k in range(j + 2, len(s1t)):
                                    if s1t[k] is not None:
                                        s1t1 = s1t[j + 1] = (fracs[j + 1] / (fracs[k] - fracs[j])) * (s1t[k] - s1t[j]) + \
                                                            s1t[j]
                                        break
                            if s2t1 is None:
                                for k in range(j + 2, len(s2t)):
                                    if s2t[k] is not None:
                                        s2t1 = s2t[j + 1] = (fracs[j + 1] / (fracs[k] - fracs[j])) * (s2t[k] - s2t[j]) + \
                                                            s2t[j]
                                        break
        ...
        if segType == ENERGY_VAR:
            # UA = sum(ua)
            rhs[eqnNo] = (UA - np.sum(ua)) / self.scaleFactorUA
            eqnNo += 1
        ...
        for i in range(nuSegments):
            ...
            if self.usePinchEqn[i]:
                ...
                if segType == ENERGY_VAR:
                    ...
                    j[eqnNo][self.uaIndex[i]] = (-self.CalcLMTD(S1T[i], S2T[i], S1T[i + 1],
                                                                S2T[i + 1])) / self.scaleFactorQ
                else:
                    # rhs(eqnNo) = duty(i) - UA / side1.numSegments * CalcLmtd(S1T(i), S2T(i), S1T(i + 1), S2T(i + 1))
                    j[eqnNo][self.UAIndex] = (-self.CalcLMTD(S1T[i], S2T[i], S1T[i + 1],
                                                             S2T[i + 1]) / nuSegments) / self.scaleFactorQ
...
class MultiSidedHeatExchangerOp(EquationSolver.EquationBasedOp):
    ...
    def __setstate__(self, oldState):
        ...
        if 'customSolver' in self.__dict__:
    ...
    def CleanUp(self):
        ...
        for signal in list(self.estimates.values()):
            signal.CleanUp()

        if hasattr(self, 'customSolver') and self.customSolver is not None:
            try:
        ...
        if paramName == NUSIDES_PAR:
            ...
            if val is None:
                if nuSides > 0:
                    ...
                    self.parameters[
                        COUNTER_CURRENT_PAR + str(0)] = False  # For consistency, make reference side cocurrent
                else:
    ...
    def UpdateStructure(self):
        ...
        if nuSides is None or nuSegments is None:
            # Can't do much
            return
        ...
        for i in range(currNuSides, nuSides, -1):
            ...
            try:
                for obj in list(self.signals.values()):
        ...
        for i in range(currNuHTransfer, nuSides):
            ...
            for j in range(i + 1):
                ...
                if j == i:
                    ...
                    self.BorrowChildPort(hTransfer.GetPort(DELTAT_PORT + '_In'),
                                         DELTAT_PORT + str(j) + '_' + str(i) + '_In')
                    self.BorrowChildPort(hTransfer.GetPort(DELTAT_PORT + '_Out'),
                                         DELTAT_PORT + str(j) + '_' + str(i) + '_Out')
        ...
        for hTransfer in self._hTransferList:
            ...
            try:
                ...
                if var is not None:
                    qTot = np.sum(var)
                    self.convRes['Q%i_%i' % (idx0, idx1)] = (qTot / nuSegments) * np.ones(nuSegments, dtype=float)

                var = self.convRes.get('UA%i_%i' % (idx0, idx1), None)
                if var is not None:
                    self.convRes['ua%i_%i' % (idx0, idx1)] = (var / nuSegments) * np.ones(nuSegments, dtype=float)

                if hTransfer.duty is not None:
                    qTot = np.sum(hTransfer.duty)
                    hTransfer.duty = (qTot / nuSegments) * np.ones(nuSegments, dtype=float)

                if hTransfer._UA is not None:
                    self.ua = (hTransfer._UA / nuSegments) * np.ones(nuSegments, dtype=float)
    ...
    def SolveBalance(self):
        ...
        h1 = np.concatenate((balVars[H1_IDX, :mNuSide], balVars[H1_IDX, mNuSide + 1:]))
        mf = np.concatenate((balVars[MF_IDX, :mNuSide], balVars[MF_IDX, mNuSide + 1:]))
        s = np.concatenate((mySign[:mNuSide], mySign[mNuSide + 1:]))
    ...
    def SolveProfilesFromBalance(self, doQTransfer=True, doAllProfiles=True):
        ...
        isColdVec = np.zeros(nuSides, dtype=int)

        # Should it solve all the profiles and do PH flashes in each segment??
        if doAllProfiles:
            calcProf = np.ones(nuSides, dtype=int)
        else:
            calcProf = np.zeros(nuSides, dtype=int)
        ...
        for row, col in self.missingFlashIdx:
            ...
            if col == H0_IDX:
                p, h = p0Vec[row], h0Vec[row]
            else:
                p, h = p1Vec[row], h1Vec[row]
        ...
        isColdVec[:] = np.where((h1Vec - h0Vec) * s > 0.0, 1, 0)
        ...
        if findPhCh:
            ...
            qPhChLst.sort()
            tempQArr = np.abs(CreateLinearDistArray(nuSegments + 1, 0.0, qVec[0]))

            # Find phase changes for deciding how to divide segments
            lastIdx = 0
            for val in qPhChLst:
                for i in range(lastIdx, nuSegments):
                    if tempQArr[i] <= val <= tempQArr[i + 1]:
        ...
        for nuSide in range(nuSides):
            ...
            if findPhCh:
                side.HArray = np.sign(h1Vec[nuSide] - h0Vec[nuSide]) * tempQArr / mfVec[nuSide] + h0Vec[nuSide]
                dp = p1Vec[nuSide] - p0Vec[nuSide]
                side.PArray = p0Vec[nuSide] * np.ones(nuSegments + 1, dtype=float) + normSegs * dp
            else:
            ...
            if doAllProfiles or calcProf[nuSide]:
                side.TArray = np.zeros(nuSegments + 1, dtype=float)
        ...
        qTrMatrixCt = np.zeros((nuSides - 1, nuSides - 1), dtype=float)
        ...
        qTrMatrix = np.linalg.solve(np.array(qTrMatrixCt), -np.array(qVec[:-1]))
        hTrIdx = -1
        for hTransfer in self._hTransferList:
            if hTransfer.UseInCalculations():
                hTrIdx += 1
                ...
                if findPhCh:
                    ...
                    duty = np.sign(qTrMatrix[hTrIdx]) * np.abs(np.array(self._sides[0].QArray, dtype=float))
                else:
                    qPerSeg = qTrMatrix[hTrIdx] / nuSegments
                    duty = hTransfer.duty = np.ones(nuSegments, dtype=float) * qPerSeg

                # lmtd = hTransfer.lmtd = np.zeros(nuSegments, dtype=float)
                ua = hTransfer.ua = np.zeros(nuSegments, dtype=float)

                lmtd = list(map(hTransfer.CalcLMTD, S1T[:-1], S2T[:-1], S1T[1:], S2T[1:]))
                lmtd = hTransfer.lmtd = np.array(lmtd, dtype=float)
                # for i in range(nuSegments):
    ...
    def SolveFromBalance(self, doQTransfer=True):
        ...
        h0Vec = np.zeros(nuSides, dtype=float)
        h1Vec = np.zeros(nuSides, dtype=float)
        p0Vec = np.zeros(nuSides, dtype=float)
        p1Vec = np.zeros(nuSides, dtype=float)
        t0Vec = np.zeros(nuSides, dtype=float)
        t1Vec = np.zeros(nuSides, dtype=float)
        vf0Vec = np.zeros(nuSides, dtype=float)
        vf1Vec = np.zeros(nuSides, dtype=float)
        qVec = np.zeros(nuSides, dtype=float)
        moleFlowVec = np.zeros(nuSides, dtype=float)
        isColdVec = np.zeros(nuSides, dtype=int)
        isCoCurrVec = np.zeros(nuSides, dtype=int)
        fracsVec = []
        ...
        for nuSide in range(nuSides):
            ...
            if findPhCh:
                side.HArray = np.sign(h1Vec[nuSide] - h0Vec[nuSide]) * tempQArr / moleFlowVec[nuSide] + h0Vec[
                    nuSide]
                dp = p1Vec[nuSide] - p0Vec[nuSide]
                side.PArray = p0Vec[nuSide] * np.ones(nuSegments + 1, dtype=float) + normSegs * dp
            else:
            ...
            side.QArray = np.zeros(nuSegments, dtype=float)
            side.TArray = np.zeros(nuSegments + 1, dtype=float)
            for seg in range(nuSegments):
                # QArray is kept (for different reasons) in kJ/h, hence hte 3.6 conversion is not needed
    ...
    def SolveForMissingMoleFlow(self):
        ...
        try:
            ...
            dh = np.array(HOutVec, dtype=float) - np.array(HInVec, dtype=float)
            acum = 0.0
            missIdx = None
            for i in range(nuSides):
                if moleFlowVec[i] is not None:
    ...
    def LoadPropVectors(self, useEst=False):
        ...
        # Dimension some arrays
        balVars = self.balanceVars = np.zeros((3, nuSides), dtype=float)
        s = self.sign = np.ones(nuSides, dtype=float)
        h0Vec = self.h0Vec = np.zeros(nuSides, dtype=float)
        h1Vec = self.h1Vec = np.zeros(nuSides, dtype=float)
        p0Vec = self.p0Vec = np.zeros(nuSides, dtype=float)
        p1Vec = self.p1Vec = np.zeros(nuSides, dtype=float)
        t0Vec = self.t0Vec = np.zeros(nuSides, dtype=float)
        t1Vec = self.t1Vec = np.zeros(nuSides, dtype=float)
        vf0Vec = self.vf0Vec = np.zeros(nuSides, dtype=float)
        vf1Vec = self.vf1Vec = np.zeros(nuSides, dtype=float)
        mfVec = self.mfVec = np.zeros(nuSides, dtype=float)
        fracsVec = self.fracsVec = []
        fracsObjVec = self.fracsObjVec = []
        propsObjVec = self.propsObjVec = []
        ...
        for i in range(nuSides):
            for j in range(i):
                if not t0Vec[j] and not t0Vec[i]:
                    ...
                    val = prop.GetValue()
                    status = prop.GetCalcStatus()
                    if val is not None and (status & (FIXED_V | PASSED_V)):
        ...
        self.sign = np.where(counterCurrVec, -1.0, 1.0)
        ...
        self.scaleFactorH = max(8000.0, max(np.abs(h0Vec)))
        self.scaleFactorF = max(100.0, max(np.abs(mfVec)))
    ...
    def CheckTCross(self):
        ...
        if nuSides == 2:
                ...
                if dt1 * dt2 < 0.0:
                    ...
                    if tArr0 is not None and tAtt1 is not None:
                        ...
                        if min(deltas) < 0.0 < max(deltas):
                            ...
                            if hIn_s0 is not None and hOut_s0 is not None:
                                zeroiscold = hIn_s0 < hOut_s0
                                if (zeroiscold and max(deltas) > 0.0) or (not zeroiscold and min(deltas) < 0.0):
                                    self.unitOpMessage = ('TemperatureCross', (dt1, dt2, self.GetPath()))
                                    if not self.IsForgetting():
                                        self.InfoMessage('TemperatureCross', (dt1, dt2, self.GetPath()),
            ...
            if min(outTLst) < min(inTLst) or max(outTLst) > max(inTLst):
                ...
                if not self.IsForgetting():
                    self.InfoMessage('TemperatureCross',
                                     (min(outTLst) - min(inTLst), -max(outTLst) + max(inTLst), self.GetPath()),
                                     MessageHandler.errorMessage)
            else:
                try:
                    if self.converged:
            ...
            if s0.GetIsCoCurrent() == s1.GetIsCoCurrent():
                ...
                if nuNones == 1:
                    ...
                    if idxMax == 0 or idxMax == 1:  # hot side is s0
                        ...
                        elif lstT[0] is None and lstT[3] is None:
                            TInVec[i] = TOutVec[i] * 1.1  # higher
                            TOutVec[i + 1] = (TInVec[i + 1] + TOutVec[i]) / 2.0  # half way
                        elif lstT[1] is None and lstT[2] is None:
                            TOutVec[i] = (TInVec[i] + TOutVec[i + 1]) / 2.0  # half way
                            TInVec[i + 1] = TOutVec[i + 1] * 0.9  # lower
                        elif lstT[1] is None and lstT[3] is None:
    ...
    def StoreConvResults(self):
        ...
        try:
            ...
            for i in range(nuSides):
                ...
                self.convRes['T%i' % i] = np.array(side.TArray, dtype=float)
                self.convRes['H%i' % i] = np.array(side.HArray, dtype=float)
                self.convRes['Q%i' % i] = np.array(side.QArray, dtype=float)
            ...
            for hTransfer in self._hTransferList:
                ...
                self.convRes['Q%i_%i' % (idx0, idx1)] = np.array(hTransfer.duty, dtype=float)
                self.convRes['ua%i_%i' % (idx0, idx1)] = np.array(hTransfer.ua, dtype=float)
    ...
    def RetrieveConvResults(self):
        ...
        try:
            ...
            for i in range(nuSides):
                ...
                side.TArray = np.array(self.convRes['T%i' % i], dtype=float)

                tempDict['H%i' % i] = side.HArray
                side.HArray = np.array(self.convRes['H%i' % i], dtype=float)

                tempDict['Q%i' % i] = side.QArray
                side.QArray = np.array(self.convRes['Q%i' % i], dtype=float)
            ...
            for hTransfer in self._hTransferList:
                ...
                hTransfer.duty = np.array(self.convRes['Q%i_%i' % (idx0, idx1)], dtype=float)

                tempDict['ua%i_%i' % (idx0, idx1)] = hTransfer.ua
                hTransfer.ua = np.array(self.convRes['ua%i_%i' % (idx0, idx1)], dtype=float)
    ...
    def GetContents(self):
        ...
        for k, v in list(self.signals.items()):
            results.append((k, v))
        for k, v in list(self.estimates.items()):
            results.append((k, v))
        return results
    ...
    def LinkToObject(self, obj, name):
        ...
        if isinstance(obj, HXObject):
            if isinstance(obj, EstimatePerSide):
                ...
                if self.estimates.get(name, None) is not None:
                    raise SimError('CantAddObject', (name, self.GetPath()))
                self.estimates[name] = obj
            else:
                # Redundant check, but just in case
                if self.signals.get(name, None) is not None:
                    raise SimError('CantAddObject', (name, self.GetPath()))
...
class CompositeSide(object):
    ...
    def InterpolateFromVectors(self, val, fromVec, mapVec, startIn=0):
        ...
        if length != len(mapVec):
            raise SimError('CantInterpolate', (self.hx.GetPath()))
    ...
    def UpdateProfiles(self):
        ...
        vals = np.array(sides[0].TArray, dtype=float)
        baseLen = len(vals)
        revIdx = list(range(baseLen))
        revIdx.reverse()

        tTot = None
        qTot = np.zeros(1, dtype=float)
        for side in sides:
            ...
            tSide = np.array(side.TArray, dtype=float)  # K
            qSide = np.array(side.QArray, dtype=float)  # kJ/h
            if not fwd:
                tSide = np.take(tSide, revIdx)
                qSide = np.take(qSide, revIdx[1:])

            qSide = qSide / 3.6  # J/s
            qSide = np.abs(np.add.accumulate(qSide))

            if tTot is None:
                tTot = tSide
                qTot = np.concatenate((qTot, qSide))
            else:
                in0 = tTot[0]
                end0 = tTot[-1]
                in1 = tSide[0]
                ...
                if in0 == in1 and end0 == end1:
                    # Identical sides
                    tTot = np.array([in0, end1], dtype=float)
                    qTot = np.array([qTot[0], qTot[-1] + qSide[-1]], dtype=float)

                elif end0 <= in1:
                    # Just link next side after this one
                    tTot = np.concatenate((tTot, tSide[1:]))
                    qSide += qTot[-1]
                    qTot = np.concatenate((qTot, qSide))
                    ...
                    if end0 <= end1:
                        ...
                        for i in range(1, len(tSide)):
                            if tSide[i] == end0:
                                ...
                                qTot = np.concatenate((qTot, qSide[i - off:]))
                                tTot = np.concatenate((tTot, tSide[i:]))
                                break
                            elif tSide[i] > end0:
                                ...
                                tTot = np.concatenate((tTot, np.array([end0], dtype=float), tSide[i:]))
                                qTot = np.concatenate((qTot, np.array([q], dtype=float), qSide[-off + i:]))
                                break
                    else:
                        idxIn1TouchTot = None
                        for i in range(1, len(tTot)):
                            if tTot[i] == in1:
                                ...
                                frac = (in1 - tTot[i - 1]) / (tTot[i] - tTot[i - 1])
                                q = qTot[i - 1] + frac * (qTot[i] - qTot[i - 1])
                                qTot = np.concatenate((qTot[:i], np.array([q], dtype=float), qTot[i:]))
                                tTot = np.concatenate((tTot[:i], np.array([in1], dtype=float), tTot[i:]))
                                idxIn1TouchTot = i
                                break

                        for i in range(idxIn1TouchTot, len(tTot)):
                            if tTot[i] == end1:
                                tTot = np.concatenate((tTot[:idxIn1TouchTot], tTot[i:]))
                                qTot = np.concatenate((qTot[:idxIn1TouchTot], qTot[i:]))
                                ...
                                tTot = np.concatenate(
                                    (tTot[:idxIn1TouchTot + 1], np.array([end1], dtype=float), tTot[i:]))
                                qTot = np.concatenate((qTot[:idxIn1TouchTot + 1], np.array([q], dtype=float), qTot[i:]))
                                qTot[idxIn1TouchTot + 1:] += qSide[-1]
                                break
    ...
    def MergeData(self, other):
        ...
        try:
            ...
            t1 = np.zeros(maxLen, dtype=float)
            q1 = np.zeros(maxLen, dtype=float)

            longQ = np.concatenate((myQ, otherQ))
            longQ = list(longQ)
            longQ.sort()
...
class PropertyPerSide(HXObject_Side):
    ...
    def HXPortName(self):
        if self.atInlet:
            return 'Variable%sIn_%d_%s' % (self.varType, self.sideIdx, self.name)
        else:
            return 'Variable%sOut_%d_%s' % (self.varType, self.sideIdx, self.name)
...
class EnergySideVar(PropertyPerSide):
    ...
    def SolveFromPortValues(self):
        ...
        try:
            ...
            if h0 is not None and h1 is not None and mf is not None:
...
class EstimateTemperature(EstimatePerSide):
    ...
    def HXPortName(self):
        if self.atInlet:
            return 'EstimateTIn_%d_%s' % (self.sideIdx, self.name)
        else:
            return 'EstimateTOut_%d_%s' % (self.sideIdx, self.name)
...
class HXObject_CompositeSideUA(HXObject_CompositeSide):
    ...
    def GetCurrentHXValue(self):
        ...
        try:
            ...
            t0 = hx.hComposite.T
            t1 = hx.cComposite.T
            duty = np.array(list(map(operator.sub, hx.hComposite.Q[1:], hx.hComposite.Q[:-1])), dtype=float)
            lmtd = np.array(list(map(CalcLMTD, t0[:-1], t1[:-1], t0[1:], t1[1:])), dtype=float)
            ua = duty / lmtd
    ...
    def GetCurrentHXValue(self):
        ...
        try:
            ...
            if value is not None:
                ...
                return np.sum(value) * self.eneSign
        except:
            return None
...
class TemperatureApproachVar(HXTranferVar):
    ...
    def Reset(self):
        ...
        if counter0 == counter1:
            if t0In is not None and t1In is not None:
                if counter0:
                    self.idxCantUse.append(nuSegments - 1)
                else:
                    self.idxCantUse.append(0)
            if t0Out is not None and t1Out is not None:
                if counter0:
                    self.idxCantUse.append(0)
                else:
                    self.idxCantUse.append(nuSegments - 1)
        else:
            if t0In is not None and t1Out is not None:
                if counter0:
                    self.idxCantUse.append(nuSegments - 1)
                else:
                    self.idxCantUse.append(0)
            if t0Out is not None and t1In is not None:
                if counter0:
                    self.idxCantUse.append(0)
                else:
                    self.idxCantUse.append(nuSegments - 1)
