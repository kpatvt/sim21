...
from sim21.old.unitop import Mixer
...
VALID_UNIT_OPERATIONS = ['SimpleFlash', 'MixAndFlash']
KEY_CMP_PAR = "KeyCmp"
...
class SimpleFlash(UnitOperations.UnitOperation):
    ...
    def Solve(self):
        ...
        idxResults = list(range(nuLiqPorts))
        for pName, resIdx in list(portToResultMap.items()):
            ...
            if cmpName is not None:
                ...
                if cmpName in cmpNames:
                    ...
                    if idxPhase == 0:
                        ...
                        for k in list(portToResultMap.keys()):
        ...
        for pName, resIdx in list(portToResultMap.items()):
            ...
            idx = resIdx
            if fMoles is not None and feedPort is flashPort:
        ...
        for port in outletPorts:
            if port != vapPort and port.GetPropValue(VPFRAC_VAR) is None:
                port.SetPropValue(VPFRAC_VAR, 0.0, CALCULATED_V)
...
class MixAndFlash(UnitOperations.UnitOperation):
    ...
    def __init__(self, initScript=None):
        ...
        for portName in list(ports.keys()):
    ...
    def ValidateOk(self):
        """True if the uo is ready to be calculated"""
        if self.GetParameterValue(NULIQPH_PAR) <= 0:
            return 0
        if self.parameters[NUSTIN_PAR] <= 0:
            return 0
        if not self.GetThermo():
            return 0
