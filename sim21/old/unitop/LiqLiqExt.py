...
from sim21.old.unitop import UnitOperations, Flash, Balance
...
class LiqLiqEx(Flowsheet.Flowsheet):
    ...
    def UpdateFeedIsLessDense(self):
        ...
        try:
            ...
            if self.feedIsLessDense != feedIsLessDense:
                self.reconnect = 1
            else:
                self.reconnect = 0
            self.topPortOutName = L_PORT + str(feedIsLessDense)
    ...
    def InitStages(self):
        ...
        fCmp = np.array(fPort.GetCompositionValues(), dtype=float)
        ...
        sCmp = np.array(sPort.GetCompositionValues(), dtype=float)
        ...
        try:
            ...
            (out0Cmp, out1Cmp) = (np.array(out0Port.GetCompositionValues(), dtype=float), np.array(out1Port.GetCompositionValues(), dtype=float))
        ...
        lstLiqRem = []
        nuCmps = len(fFlow)
        liqRemaining = np.zeros((nuCmps, nuStages), dtype=float)
        for cmpNo in range(nuCmps):
            ...
            powSeries = np.multiply.accumulate(np.ones(nuStages) * E)
            denom = sum(powSeries) + 1
            numer = np.concatenate(([1.], powSeries[:-1]))  # Insert a 1 at the beginning and get rid of last number
            numer = np.add.accumulate(numer)
        ...
        for i in range(nuStages - 1, 0, -1):
            ...
            in1Cmp = np.array(in1Port.GetCompositionValues(), dtype=float)
