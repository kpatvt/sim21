...
from sim21.old.data.eqn import eval_eqn_int
from sim21.old.solver.Messages import MessageHandler
from sim21.old.solver.Variables import *
from sim21.old.unitop import UnitOperations, Properties, Stream
...
class Stage(object):
    ...
    def Minus(self, numStages):
        ...
        for feed in list(stages[firstStage].feeds.values()):
        ...
        # Check for feeds from pump arounds
        for feed in list(stages[lastStage].feeds.values()):
            pa = feed.pumpFromDraw
            if pa and isinstance(pa, PumpAround):
                # A pump around. Cannot delete unless the whole thing is deleted
                if pa.stage.number < firstStage or pa.stage.number > lastStage:
        ...
        for i in range(firstStage + 1, lastNormal + 1):
            ...
            # Check for feeds from pump arounds
            for feed in list(stages[i].feeds.values()):
                pa = feed.pumpFromDraw
                if pa and isinstance(pa, PumpAround):
                    # A pump around. Cannot delete unless the whole thing is deleted
                    if pa.stage.number < firstStage or pa.stage.number > lastStage:
    ...
    def LinkToObject(self, obj, name):
        ...
        if isinstance(obj, Feed):
            ...
            for draw in list(self.liqDraws.values()) + list(self.vapDraws.values()):
                if name in draw.estimates:
                    raise SimError('CantAddToStage', (name, self.number, self.tower.GetPath()))
            for clone in list(self.liqClones.values()) + list(self.vapClones.values()):
    ...
    def GetContents(self):
        ...
        for key in list(self.estimates.keys()):
    ...
    def Reset(self, initMode=SCRATCH_INIT):
        ...
        for feed in list(self.feeds.values()) + list(self.qfeeds.values()):
            feed.Reset()
    ...
    def NumberInner(self, cntActiveOnly=0):
        ...
        if cntActiveOnly:
            ...
            if self.type == TOP_STAGE and len(self.vapDraws):
                # use an arbitrary draw as the basis for stage flow ratios
                list(self.vapDraws.values())[0].isBasis = 1
            elif self.type == BOTTOM_STAGE and len(self.liqDraws):
                # use an arbitrary draw as the basis for stage flow ratios
                list(self.liqDraws.values())[0].isBasis = 1
            else:
                # either normal stage or no appropriate draws
                nEqns += 1
    ...
    def NumberSpecs(self):
        ...
        if self.tower.converged == 0:
            ...
            for qfeed in list(self.qfeeds.values()):
                if qfeed.port.GetValue() is not None:
                    self.userSpecs += 1

        for qfeed in list(self.qfeeds.values()):
            if qfeed.port.GetValue() is None:
                numSpecs -= 1
                break
    ...
    def TotalQFlow(self):
        ...
        for feed in list(self.qfeeds.values()):
            feedQ = feed.TotalQFlow()
            if feedQ is not None:
                q += feedQ
        return q
    ...
    def EstimateTemperature(self):
        ...
        """
        for estimate in list(self.estimates.values()):
    ...
    def SpecErrors(self):
        ...
        names = list(self.vapClones.keys())
        names.sort()
        for name in names:
            results.extend(self.vapClones[name].SpecErrors())

        names = list(self.liqClones.keys())
        names.sort()
        for name in names:
    ...
    def TotalDrawFlow(self):
        ...
        """
        total = 0.
        for draw in list(self.vapDraws.values()) + list(self.liqDraws.values()):
            total += draw.flow
    ...
    def EstimateReflux(self):
        ...
        """
        for est in list(self.estimates.values()):
            if est.type == REFLUX:
    ...
    def ReadyToSolve(self):
        ...
        for draw in list(self.liqDraws.values()):
            if self.subCool:
                draw.isSubCooled = True
            draw.ReadyToSolve()

        for draw in list(self.vapDraws.values()):
            if self.subCool:
                draw.isSubCooled = True
            draw.ReadyToSolve()

        for clone in list(self.liqClones.values()):
            if self.subCool:
                clone.isSubCooled = True
            clone.ReadyToSolve()

        for clone in list(self.vapClones.values()):
            if self.subCool:
                clone.isSubCooled = True
            clone.ReadyToSolve()
        ...
        if self.subCool:
            try:
                ...
                if dt is not None and dt > 0.0:
                    for draw in list(self.vapDraws.values()) + list(self.vapClones.values()):
    ...
    def CloneContents(self, clone):
        ...
        for name, obj in list(self.feeds.items()):
        ...
        for name, obj in list(self.liqDraws.items()):
            prevObj = clone.GetObject(name)
            if not prevObj:
                objClone = obj.Clone()
                clone.AddObject(objClone, name)
        ...
        for name, obj in list(self.vapDraws.items()):
            prevObj = clone.GetObject(name)
            if not prevObj:
                objClone = obj.Clone()
                clone.AddObject(objClone, name)
        ...
        for name, obj in list(self.liqClones.items()):
            prevObj = clone.GetObject(name)
            if not prevObj:
                objClone = obj.Clone()
                clone.AddObject(objClone, name)
        ...
        for name, obj in list(self.vapClones.items()):
            prevObj = clone.GetObject(name)
            if not prevObj:
                objClone = obj.Clone()
                clone.AddObject(objClone, name)
        ...
        for name, obj in list(self.qfeeds.items()):
            prevObj = clone.GetObject(name)
            if name[-4:] != '_paQ':
                if not prevObj:
                    objClone = obj.Clone()
        ...
        for name, obj in list(self.specs.items()):
            prevObj = clone.GetObject(name)
            if not prevObj:
                objClone = obj.Clone()
                clone.AddObject(objClone, name)
        ...
        for name, obj in list(self.estimates.items()):
            prevObj = clone.GetObject(name)
            if not prevObj:
                objClone = obj.Clone()
                clone.AddObject(objClone, name)
...
class DegSubCooling(StageObject):
    ...
    def ReadyToSolve(self):
        ...
        if self.value is None:
            ...
            for draw in list(stage.liqDraws.values()) + list(stage.liqClones.values()):
                aLiqT = draw.port.GetPropValue(T_VAR)
                if aLiqT:
                    self.subCooledT = aLiqT
                    return True

            for draw in list(stage.vapDraws.values()) + list(stage.vapClones.values()):
                aVapT = draw.port.GetPropValue(T_VAR)
                if aVapT:
                    self.subCooledT = aVapT
                    return True

            if aLiqT is None and aVapT is None:
                for spec in list(stage.specs.values()):
...
class Feed(StageObject):
    ...
    def MoleArray(self):
        ...
        if moles == 0.0:
            return np.zeros(self.stage.tower.numCompounds, float)
...
class Draw(StageObject):
    ...
    def AddObject(self, obj, name):
        ...
        if isinstance(obj, Estimate):
            if name in self.estimates:
                ...
                for draw in list(stage.liqDraws.values()) + list(stage.vapDraws.values()):
                    if name in draw.estimates:
                        raise SimError('CantAddToStage', (name, stage.number, stage.tower.GetPath()))
                for clone in list(stage.liqClones.values()) + list(stage.vapClones.values()):
                    if name in clone.estimates:
                        raise SimError('CantAddToStage', (name, stage.number, stage.tower.GetPath()))
            ...
            for spec in list(self.drawSpecs.values()):
    ...
    def Reset(self):
        ...
        if estFlow is not None:
            self.flow = estFlow
        elif moleFlow is not None:
            self.flow = moleFlow
        if self.zeroFlow:
        ...
        if connection and connection.GetParent() == self.stage.tower:
            ...
            (toNumber, toName) = connection.GetName().split('_', 2)[1:]
            ...
            elif self.pumpToFeed.flow is not None:
    ...
    def ReadyToSolve(self):
        ...
        for spec in self.portSpecs:
            ...
            if hasattr(spec, 'port') and spec.SpecValue() is None:
                self.portSpecs = []
                break
    ...
    def CloneContents(self, clone):
        ...
        for name, obj in list(self.drawSpecs.items()):
            prevObj = clone.GetObject(name)
            if not prevObj:
                objClone = obj.Clone()
                clone.AddObject(objClone, name)
...
class PumpAround(Draw):
    ...
    def NumberSpecs(self):
        ...
        portFlow = self.port.GetPropValue(MOLEFLOW_VAR)
        massFlow = self.port.GetPropValue(MASSFLOW_VAR)
        if portFlow is None and massFlow is None:
...
class WaterDraw(Draw):
    ...
    def InitializeKFactors(self):
        ...
        f = np.zeros(tower.numCompounds, float)
        ...
        try:
            self.liq1Frac = results.phaseFractions[1] / (results.phaseFractions[1] + results.phaseFractions[2])
        except:
            self.liq1Frac = None
        ...
        try:
            self.oldFlows, self.oldFlow = np.array(self.moleFlows, float), self.flow
        except:
            pass
        self.f = f
        ...
        if newFlow == 0.0 and self.flow:
            ...
            self.moleFlows = (self.moleFlows * oneMinusDamp +
                              np.array(results.phaseComposition[2]) * newFlow * damping)

            # Get properties at damped composition
            vals = thAdmin.GetProperties(prov, case, (T_VAR, T), (P_VAR, tower.P[stageNo]),
                                         LIQUID_PHASE, self.moleFlows / np.sum(self.moleFlows), (H_VAR, CP_VAR))
            ...
            self.flow = newFlow
            self.moleFlows = np.array(results.phaseComposition[2]) * self.flow
    ...
    def AssignResultToPort(self):
        ...
        if self.moleFlows:
            stageNo = self.stage.number
            tower = self.stage.tower
            self.port.SetCompositionValues(self.moleFlows / np.sum(self.moleFlows), CALCULATED_V)

    def MoleFracs(self):
        """return the current mole fractions"""
        return self.moleFlows / np.sum(self.moleFlows)
    ...
    def StoreConvResults(self):
        ...
        try:
            ...
            self.convRes['moleFlows'] = np.array(self.moleFlows, float)
    ...
    def RetrieveConvResults(self):
        ...
        try:
            ...
            for key in self.convRes:
                ...
                if key == 'moleFlows':
                    self.__dict__[key] = np.array(self.convRes[key], float)
                else:
...
class StdVolRecoverySpec(ComponentSpec):
    ...
    def GetCurrentTowerValue(self):
        ...
        feeds = np.sum(tower.f, 0)  # kmole/h per compound
        totFeedFlow = np.sum(feeds)  # kmole/h
...
class SpecialPropertySpec(DrawSpec):
    ...
    def __init__(self, propName):
        ...
        if propName in (BUBBLEPOINT_VAR, DEWPOINT_VAR, WATERDEWPOINT_VAR, CO2VSE_VAR, CO2LSE_VAR, HVAPCTEP_VAR):
            ...
            if propName == KINEMATICVISCOSITY_VAR:
                self.callIdx = 0
            else:
                self.callIdx = 1
...
class Tower(UnitOperations.UnitOperation):
    ...
    def StoreConvResults(self):
        ...
        try:
            # The keys of convRes must exactly match the names of the attributes in the tower
            self.convRes['flowMatrix'] = np.array(self.flowMatrix, float)
            self.convRes['T'] = np.array(self.T, float)
            self.convRes['V'] = np.array(self.V, float)
            self.convRes['L'] = np.array(self.L, float)
            self.convRes['l'] = np.array(self.l, float)
            self.convRes['v'] = np.array(self.v, float)

            self.convRes['A'] = np.array(self.A, float)
            self.convRes['B'] = np.array(self.B, float)
            self.convRes['alpha'] = np.array(self.alpha, float)

            self.convRes['hlModel'] = np.array(self.hlModel, float)
            self.convRes['hvModel'] = np.array(self.hvModel, float)
            self.convRes['jacobian'] = np.array(self.jacobian, float)
            self.convRes['logSFactors'] = np.array(self.logSFactors, float)
    ...
    def ThermoChanged(self, thCaseObj):
        ...
        for stage in self.stages:
            for draw in list(stage.liqDraws.values()) + list(stage.vapDraws.values()):
                draw.Reset()
            for draw in list(stage.liqClones.values()) + list(stage.vapClones.values()):
                draw.Reset()
    ...
    def UpdateResultsStagesRemoved(self, firstStage, lastStage):
        try:
            ...
            for varName in ['T', 'L', 'V',
                ...
                if var:
                    newVar = np.zeros(len(var) - nuStagesRem, float)
            ...
            for varName in ['v', 'l', 'alpha']:
                ...
                if var:
                    dims = np.shape(var)
                    newVar = np.zeros((dims[0] - nuStagesRem, dims[1]), float)
                    newVar[0:firstStage][:] = var[0:firstStage][:]
                    newVar[firstStage:][:] = var[lastStage + 1:][:]
                    self.__dict__[varName] = newVar
                if convRes:
            ...
            for varName in ['hlModel', 'hvModel']:
                ...
                if var:
                    dims = np.shape(var)
                    newVar = np.zeros((dims[0], dims[1] - nuStagesRem), float)
                    newVar[:, 0:firstStage] = var[:, 0:firstStage]
                    newVar[:, firstStage:] = var[:, lastStage + 1:]
                    self.__dict__[varName] = newVar
                if convRes:
            ...
            if self.v and self.V:
                self.y = np.transpose(np.transpose(self.v) / self.V)

            if self.l and self.L:
                self.x = np.transpose(np.transpose(self.l) / self.L)
    ...
    def UpdateResultsStageAdded(self, currNuStages, nuNewStage):
        try:
            ...
            for varName in ['T', 'L', 'V', 'A', 'B', 'logSFactors']:
                var = self.__dict__[varName]
                if var:
                    newVar = np.zeros(len(var) + 1, float)
            ...
            for varName in ['v', 'l', 'alpha']:
                ...
                if var:
                    dims = np.shape(var)
                    newVar = np.zeros((dims[0] + 1, dims[1]), float)
                    newVar[0:nuNewStage][:] = var[0:nuNewStage][:]
                    newVar[nuNewStage][:] = var[nuNewStage - 1][:]
                    newVar[nuNewStage + 1:][:] = var[nuNewStage:][:]
                    self.__dict__[varName] = newVar
            ...
            for varName in ['hlModel', 'hvModel']:
                ...
                if var:
                    dims = np.shape(var)
                    newVar = np.zeros((dims[0], dims[1] + 1), float)
                    newVar[:, 0:nuNewStage] = var[:, 0:nuNewStage]
                    newVar[:, nuNewStage] = var[:, nuNewStage - 1]
                    newVar[:, nuNewStage + 1:] = var[:, nuNewStage:]
                    self.__dict__[varName] = newVar
    ...
    def RemoveStages(self, firstStage, lastStage):
        ...
        if not self.dontRestartNextTime:
            try:
                self.numInnerEqns -= (lastStage - firstStage + 1)
            except:
                pass
    ...
    def PropagatePressures(self):
        ...
        for i in range(self.numStages):
            ...
            if pVal_1 is not None:
                stage.SetPressure(pVal_1)
            elif pVal_0 is not None:
    ...
    def Solve(self):
        ...
        self.mw = np.zeros(self.numCompounds, float)
        ...
        try:
            ...
            if not initMode:
                ...
                self.hlModel = None
                self.hvModel = None
                if self.isSubCool and self.subCoolDT is None:
            ...
            elif not hasattr(self, 'jacobian') or self.jacobian is None or np.shape(self.jacobian)[0] != self.numInnerEqns:
            ...
            oldErrors = np.zeros(self.numInnerEqns, float)
            oldLogSFactors = np.zeros(self.numInnerEqns + totCond + totReb, float)
            ...
            while 1:  # outer loop
                innerLoopCount = 0
                outerOldT = np.array(self.T, float)
                ...
                while innerLoopCount < maxInnerLoops:  # inner loop
                    ...
                    if self.debug:
                        ...
                        debug_file.write(txt)
                        wIdx = self.waterIdx
                        drawRatios = -3271 * np.ones(self.numStages, float)
    ...
    Iterations:.............. %i""" % (path,
        ...
        if self.converged:
            ...
            if self.subCoolT is not None and abs(self.V[0]) > 1.0E-7:
    ...
    def SolveFlowMatrix(self, logSFactors):
        ...
        """
        # check minimums
        logSFactors = np.clip(logSFactors, logTiniestValue, logLargestValue)

        # extract stripping factors and draw ratios
        Sj = np.exp(logSFactors[:self.numStages])
        drawRatios = np.exp(logSFactors[self.numStages:])

        # calculate the draw ratio terms
        RvTerm = np.ones(self.numStages, float)  # 1 + sum(Rvjn)
        RlTerm = np.ones(self.numStages, float)  # 1 + sum(Rljn)

        # pump around terms, obviously not efficient, but expedient
        paVTerm = np.zeros((self.numStages, self.numStages), float)
        paLTerm = np.zeros((self.numStages, self.numStages), float)

        # paVTerm = np.empty((self.numStages, self.numStages), float)
        # paLTerm = np.empty((self.numStages, self.numStages), float)
        ...
        if self.useEff:
            effOffset = np.zeros(self.numStages, float)  # need this predefined
            effOffsetPA = np.zeros((self.numStages, self.numStages), float)
            eff = self.eff

        rhs = np.zeros(self.numStages, float)
        for i in range(self.numCompounds):
            # create solution matrix
            flowMatrix = np.zeros((self.numStages, self.numStages), float)
            ...
            if self.useEff:
                ...
                upper = -np.ones(self.numStages, float)
        ...
        for stage in self.useStages:
            ...
            # fill in clone flows
            for clone in list(stage.liqClones.values()):
                clone.flow = self.L[stage.number]

            for clone in list(stage.vapClones.values()):
        ...
        if self.dokmix:
            try:
                ...
                for wdraw in self.waterDraws:
                    ...
                    a = wdraw.liq1Frac
                    wlnK0 = lnK[nuStage][:]
                    wlnK1 = wLnFug + np.log(y[nuStage, :]) - lnFugV[nuStage, :] - np.log(wx)
                    wK0 = np.exp(wlnK0)
                    wK1 = np.exp(wlnK1)
                    wK = (wK0 * wK1) / (a * wK1 + (1.0 - a) * wK0)
                    lnK[nuStage][:] = np.log(wK)
            except:
                pass
    ...
    def GetBubbleLnK(self, p, x):
        ...
        y = np.clip(results.phaseComposition[0], tiniestValue, largestValue)

        return t, y, np.log(y / x)
    ...
    def CalculateTemperatures(self):
        ...
        self.T += deltaT
        self.T = np.clip(self.T, 1.0, 10000.0)
    ...
    def InitOuterProperties(self):
        ...
        if hasattr(self, 'doplots'):
            if self.doplots:
                wIdx = self.waterIdx
                lnKArray = np.zeros((10, lnK.shape[0], lnK.shape[1]), float)
                for i in range(10):
                    lnKArray[i] = self.GetLnK(T + float(i), self.P, self.x, self.y)

                xtemp = np.array(self.x, float)
                xtemp[:, wIdx] *= 1.1
                xtemp = np.clip(np.transpose(np.transpose(xtemp) / np.add.reduce(xtemp, 1)), tiniestValue, largestValue)
                lnKArray2 = np.zeros((10, lnK.shape[0], lnK.shape[1]), float)
                for i in range(10):
                    lnKArray2[i] = self.GetLnK(T + float(i), self.P, xtemp, self.y)

                lnKArray3 = np.zeros((10, lnK.shape[0], lnK.shape[1]), float)
                for i in range(10):
                    xtemp2 = np.array(self.x, float)
        ...
        if not self.useOldCode:
            try:
                self.V = np.clip(self.V, smallestAllowedFlow, largestAllowedFlow)
                self.L = np.clip(self.L, smallestAllowedFlow, largestAllowedFlow)
                KbCurrent = np.exp(self.logSFactors[:self.numStages]) * self.L / self.V
                KbCurrent[0] = KbCurrent[self.totCond]
                KbCurrent[-1] = KbCurrent[-1 - self.totReb]
                lnKbCurrent = np.log(KbCurrent)
            except:
        ...
        if self.useOldCode:
            ...
            lnAlpha = np.clip(np.transpose(np.transpose(lnK) - self.A + self.B / T), logTiniestValue, logLargestValue)
            if dampingFactor < 1.0:
                self.alpha *= np.power(np.exp(lnAlpha) / self.alpha, dampingFactor)
            else:
            ...
            try:
                if lnKbCurrent is None or np.amax(np.absolute(np.exp(lnKb1) - KbCurrent)) > 20.0:
                    ...
                    if dampingFactor < 1.0:
                        ...
                        if oldB is not None:
                            oldK = self.alpha * np.exp(self.A - oldB / T)
                        else:
                            oldK = self.alpha
                        lnK = np.log(oldK * np.power(np.exp(lnK) / oldK, dampingFactor))
                    self.A = lnKb1 + self.B / T
                    lnAlpha = np.clip(np.transpose(np.transpose(lnK) - self.A + self.B / T), logTiniestValue,
                                      logLargestValue)

                    # Finally notify logSFactors about changes to Kb
                    self.logSFactors[:self.numStages] = np.log(np.exp(lnKb1) * self.V / self.L)
                else:
                    # Keep Kb constant by updating A to match it
                    if not self.A:
                        self.A = lnKb1 + self.B / T
                    else:
                        self.A = lnKbCurrent + self.B / T
        ...
        # Enthalpy model
        if self.isSubCool:
            if self.subCoolDT is not None:
                self.hlModel = self.GetEnthalpyModel(self.T - self.subCoolDT, self.P, self.x, LIQUID_PHASE,
                                                     self.hlModel)
                self.hvModel = self.GetEnthalpyModel(self.T - self.subCoolDT, self.P, self.y, VAPOUR_PHASE,
                                                     self.hvModel)
            else:
    ...
    def InnerErrors(self):
        ...
        # basic heat balance
        errors = np.zeros(self.numInnerEqns, float)
        ...
        if self.hasPumps:
            errors[:self.numStages] += np.add.reduce(hl * self.L * self.paLTerm, 1) + np.add.reduce(
                hv * self.V * self.paVTerm, 1)
        ...
        self._lastEneErrs = np.array(errors)
    ...
    def GetSpecLbls(self):
        ...
        for stage in self.stages:
            names = list(stage.vapDraws.keys())
            names.sort()
            for name in names:
            ...
            names = list(stage.liqDraws.keys())
            names.sort()
            for name in names:
            ...
            names = list(stage.vapClones.keys())
            names.sort()
            for name in names:
            ...
            names = list(stage.liqClones.keys())
            names.sort()
            for name in names:
    ...
    def CalcJacobian(self):
        ...
        jacobian = np.zeros((self.numInnerEqns, self.numInnerEqns), float)
    ...
    def Profile(self, phase, property):
        ...
        if phase == TOWER_LIQ_PHASE:
            phase = LIQUID_PHASE
            try:
                frac = np.array(self.x, float)
            except:
                return None
        elif phase == TOWER_VAP_PHASE:
            phase = VAPOUR_PHASE
            try:
                frac = np.array(self.y, float)
            except:
                return None
        else:
            return None
        ...
        if property == FRAC_VAR or property == CMPMOLEFRAC_VAR:
            ...
            mwts = np.ones(self.numStages, float)
            thCaseObj = self.GetThermo()
            thAdmin, prov, case = thCaseObj.thermoAdmin, thCaseObj.provider, thCaseObj.case
            ...
            stdMolVol = np.ones(self.numStages, float)
            thCaseObj = self.GetThermo()
            thAdmin, prov, case = thCaseObj.thermoAdmin, thCaseObj.provider, thCaseObj.case
            ...
            if waterFree:
                molVol = self.storedProfiles.get((phase, WATERFREE + MOLARV_VAR), None)
            else:
                molVol = self.storedProfiles.get((phase, MOLARV_VAR), None)
            if not molVol or len(molVol) != self.numStages:
                molVol = np.ones(self.numStages, float)
                ...
                if waterFree:
                    self.storedProfiles[(phase, WATERFREE + MOLARV_VAR)] = molVol
                else:
                    self.storedProfiles[(phase, MOLARV_VAR)] = molVol
            else:
                pass
            if phase == LIQUID_PHASE:
    ...
    def _CloneCreate(self):
        ...
        if self.initTowerObj is not None:
            try:
...
def BuildEfficienciesMatrix(inputString, numberElements, cmpNames, defaultValue=1.0):
    ...
    if isinstance(inputString, str) and '@' in inputString:
        ...
        colsNotUsed = list(range(numCompounds))
        ...
        cmpDataTokens = inputString.split('@')
        eff = np.ones((numberElements, numCompounds), float) * defaultValue
        genericEff = None
        for cmpToken in cmpDataTokens:
            ...
            if cmpName in cmpNames:
                ...
                eff[:, col] = np.clip(stageEff[:], 0.0, 1.0)
        ...
        if usesGenericEff:
            for col in colsNotUsed:
                eff[:, col] = np.clip(genericEff[:], 0.0, 1.0)

    else:
        # Use same eff for all compounds
        stageEff = ParseInputArray(inputString, numberElements)
        stageEff = np.clip(stageEff, 0.0, 1.0)
        eff = np.reshape(np.repeat(stageEff, numCompounds), (numberElements, numCompounds))
...
def ParseInputArray(inputString, numberElements, defaultValue=1.0):
    ...
    if isinstance(inputString, float) or isinstance(inputString, int):
        return np.ones(numberElements, float) * inputString
    ...
    tokens = inputString.split()
    if len(tokens) == 1:
        return np.ones(numberElements, float) * float(tokens[0])
    returnValues = np.ones(numberElements, float) * defaultValue
...
class ProfileObj(object):
    ...
    def CleanUp(self):
        ...
        self.parent = None
        self.type = None
        list(map(ProfileProperty.CleanUp, self.props))
    ...
    def SetSize(self, size):
        ...
        # Delete if needed
        list(map(ProfileProperty.CleanUp, self.props[size:]))
        del self.props[size:]
        ...
        list(map(ProfileProperty.SetIndex, self.props, list(range(size))))
    ...
    def AddProperty(self, idx=-1):
        ...
        if idx == -1:
            ...
            # Re assign all the indexes
            size = len(self.props)
            list(map(ProfileProperty.SetIndex, self.props[idx:], list(range(idx, size))))
    ...
    def RemoveProperties(self, fromIdx, toIdx):
        ...
        # Re assign all the indexes
        size = len(self.props)
        list(map(ProfileProperty.SetIndex, self.props[fromIdx:], list(range(fromIdx, size))))
    ...
    def SetValues(self, values, status=CALCULATED_V):
        ...
        # Loop with map
        list(map(ProfileProperty.SetValue, self.props, values, status))

    def GetValues(self):
        """Get all the values in one single vector"""
        values = list(map(ProfileProperty.GetValue, self.props))
        return values

    def Forget(self):
        """Call th forget of each property"""
        list(map(ProfileProperty.Forget, self.props))
    ...
    def GetObject(self, desc):
        if desc[:4] == 'Item':
            ...
            return list(map(DetailedTuple, self.props))
...
class InitializeTower(object):
    ...
    def GetPath(self):
        return '%s.%s' % (self.tower.GetPath(), self.name)
    ...
    def InitializeMemberVariables(self, tower, initMode):
        ...
        if not initMode:
            ...
            tower.flowMatrix = np.zeros((tower.numStages, tower.numStages), float)
            tower.T = np.zeros(tower.numStages, float)
            tower.V = np.zeros(tower.numStages, float)
            tower.L = np.zeros(tower.numStages, float)
            tower.v = np.zeros((tower.numStages, tower.numCompounds), float)
            tower.l = np.zeros((tower.numStages, tower.numCompounds), float)
            tower.alpha = np.ones((tower.numStages, tower.numCompounds), float)
            # Can not init logSFactors yet

        # Pressure is calculated here
        tower.P = np.zeros(tower.numStages, float)
        ...
        # Feeds and draws
        tower.f = np.zeros((tower.numStages, tower.numCompounds), float)
        tower.fQ = np.zeros(tower.numStages, float)
        tower.feeds = []
        tower.draws = []
        tower.waterDraws = []
        tower.sections = []
    ...
    def LoadSubCoolingInfo(self, tower, initMode):
        ...
        if tower.stages[0].IsSubCooled():
            infovector = np.zeros(tower.numStages, float)
    ...
    def ResetDraws(self, tower, initMode):
        ...
        for stage in tower.stages:
            ...
            for feed in list(stage.feeds.values()):
        ...
        for stage in tower.stages:
            ...
            draws = list(stage.liqDraws.values()) + list(stage.vapDraws.values())
    ...
    def SolveP_LoadFinalInfo(self, tower, initMode):
        ...
        for j in range(tower.numStages):
            ...
            if p:
                ...
                np.put(tower.P, list(range(lastPStage + 1, j + 1)), np.array(intP[1:]))
                lastPStage = j
                lastP = p
            ...
            if not initMode:
                ...
                if t is None and stage.type == TOP_STAGE:
                    # Reset variables
                    aFeedTemp = None
                    lastT = None
                    if j:
                ...
                if t is not None:
                    ...
                    np.put(tower.T, list(range(lastTStage + 1, j + 1)), np.array(intT[1:]))
                    lastTStage = j
                    lastT = t
        ...
        tower.totalFeedFlow = np.add.reduce(tower.f.flat)  # used for missing draws and error scaling
        ...
        if not initMode:
            ...
            if tower.initKPower != 0:
                ...
                if tower.initKPower != 1.0:
                    tower.alpha = np.power(tower.alpha, tower.initKPower)
                ...
                if tower.waterDraws:
                    ...
                    for wDraw in tower.waterDraws:
                        ...
                        wDraw.moleFlows = np.array(results.phaseComposition[2]) * wDraw.flow
            ...
            if np.min(eff) < 1.0:
                ...
                for stage in tower.stages:
                    ...
                    if stage.type == TOP_STAGE:
                        ...
                        hasVapDraw = 0
                        for draw in list(stage.vapDraws.values()):
                            if draw.isBasis:
                                hasVapDraw = 1
                                break

                        if not hasVapDraw:
                            tower.eff[stage.number] = np.ones(tower.numCompounds, float)
                        else:
    ...
    def EstimateL_V_And_logS(self, tower, initMode):
        ...
        if not initMode:
            ...
            for stage in tower.stages:
                ...
                totalLiqDraw = 0.0
                for draw in list(stage.liqDraws.values()):
            ...
            tower.logSFactors = np.ones(tower.numInnerEqns + totCond + totReb, float)
...
def TowerDumpAlpha(tower):
    try:
        ...
        for i in range(tower.numStages):
            ...
            f.write(txt)
            for j in range(len(tower.cmpNames)):
                txt = '%*.8g ' % (length[j], np.log(tower.alpha[i, j]))
                file.write(txt)
            txt = '\n'
            f.write(txt)
        file.write('\n\n')
        ...
        for section in tower.sections:
            ...
            for stage in tower.stages[top: bottom + 1]:
                ...
                for feed in list(stage.feeds.values()):
                    ...
                    if val is None and feed.pumpFromDraw:
                ...
                for feed in list(stage.qfeeds.values()):
                    if feed.incoming:
