...
class BasicDesignParameter(object):
    ...
    def __init__(self, parent, name, infoType, propTypeName=None, options=None, dispName='', idx=0):
        if options is None:
            options = []
        self.parent = parent
        self.name = name
    ...
    def GetContents(self):
        results = [(VALUE_KEYWORD, self.value), (INFO_TYPE_KEYWORD, self.infoType), (OPTIONS_KEYWORD, self.options),
                   (DISP_NAME_KEYWORD, self.dispName)]
        return results
...
class DesignMain(object):
    ...
    def GetContents(self):
        ...
        results = [(INPUT_KEYWORD, DesignInfoWrapper(self._input, self.parent, INPUT_KEYWORD)),
                   (OUTPUT_KEYWORD, DesignInfoWrapper(self._output, self.parent, OUTPUT_KEYWORD))]
        return results
