...
def GetCompoundPropertyLists(hypoName, hypoDescs, unitSystem=None):
    ...
    convertedHypoDescs = ''
    for line in lines:
        tokens = re.split(r'\s+', line.strip(), 2)
        if len(tokens) == 3 and tokens[1] == '=':
            if tokens[0] in hypoDoubles:
                ...
                vals = re.split(r'\s+', tokens[2].strip(), 1)
