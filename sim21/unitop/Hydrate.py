...
from sim21.unitop import UnitOperations, Sensor, Stream
...
class Hydrate(UnitOperations.UnitOperation):
    ...
    def Solve(self):
        ...
        y = {}
        if "METHANE" not in CompNames:
            y[methIdx] = 0.0
        if "ETHANE" not in CompNames:
            y[ethIdx] = 0.0
        if "PROPANE" not in CompNames:
            y[propIdx] = 0.0
        if "ISOBUTANE" not in CompNames:
            y[ibutIdx] = 0.0
        if "CARBON_DIOXIDE" not in CompNames:
            y[co2Idx] = 0.0
        if "HYDROGEN_SULFIDE" not in CompNames:
            y[h2sIdx] = 0.0
        ...
        sum0 = np.sum(list(y.values()))
        sum1 = 1 - sum0  # Lump the non-hydrate components as sum1
        y[n2Idx] = sum1
        y[nc4Idx] = 0.0
        y[waterIdx] = 0.0
...
def CalculateHydrateTemperature(Press, y, NN, tol=0.001, maxIter=20, scaleFactor=1.0, Temp=20.0):
    ...
    while cnt < maxIter and not converged:
            ...
            for j in range(1, PC[i] + 1):
                for L in range(0, NOP[i] + 1):
                    if (Press == P_DB[i][j]) and (Temp > T_DB[i][j][L]) and (Temp < T_DB[i][j][L + 1]):
                        IKCalc[i][j][L] = (K[i][j][L + 1] - K[i][j][L]) / (T_DB[i][j][L + 1] - T_DB[i][j][L]) * (
                            Temp - T_DB[i][j][L]) + K[i][j][L]
                        KCalc[i] = IKCalc[i][j][L]
        ...
        for i in range(1, N + 1):
            for j in range(1, PC[i] + 1):
                for L in range(0, NOP[i] + 1):
                    ...
                    if Press < P_DB[i][1]:
                        K[i][0][L] = (K[i][1][L] - K[i][2][L]) / (P_DB[i][1] - P_DB[i][2]) * (Press - P_DB[i][1]) + \
                                     K[i][1][L]
                        T_DB[i][0][L] = L
                        if K[i][0][L] <= 0.0:
                            K[i][0][L] = 0.00001
                    ...
                    if Press > P_DB[i][PC[i]]:
                        K[i][0][L] = (K[i][PC[i] - 1][L] - K[i][PC[i]][L]) / (P_DB[i][PC[i] - 1] - P_DB[i][PC[i]]) * (
                            Press - P_DB[i][PC[i]]) + K[i][PC[i]][L]
                        T_DB[i][0][L] = L
                        if K[i][0][L] <= 0.0:
                            K[i][0][L] = 0.00001
                    ...
                    if (Press > P_DB[i][j]) and (Press < P_DB[i][j + 1]):
                        K[i][15][L] = (K[i][j + 1][L] - K[i][j][L]) / (P_DB[i][j + 1] - P_DB[i][j]) * (
                            Press - P_DB[i][j]) + K[i][j][L]
                        T_DB[i][15][L] = L
        ...
        for i in range(1, N + 1):
            yall = y
            YOK[i] = yall[i] / KCalc[i]

        sum0 = np.sum(list(YOK.values()))
        sum1 = np.sum(list(yall.values()))

        # --------------------------------------------------------------------------------------------------------
        # Taking Ethane as dominant component
        #        NN = 3
        KNew[NN] = sum0 * KCalc[NN]
        for i in range(1, PC[NN] + 1):
            if Press == P_DB[NN][i]:
                for j in range(0, NOP[NN] + 1):
                    if K[NN][i][j] < KNew[NN] < K[NN][i][j + 1]:
                        II = j
