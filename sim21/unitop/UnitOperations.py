...
from sim21.solver import Variables
from sim21.solver import Error
from sim21.solver.Messages import MessageHandler
from sim21.thermo.ThermoAdmin import ThermoCase, ThermoAdmin
from sim21.design import Design
import re
import io
import time
import copy
...
class UnitOperation(object):
    ...
    def GetContents(self):
        ...
        for key in list(self.chUODict.keys()):
        ...
        for i in list(self.designObjects.items()):
            result.append(i)
    ...
    def CleanUp(self):
        ...
        try:
            ...
            for i in list(self.chUODict.keys()):
            ...
            if hasattr(self, 'designObjects'):
                for obj in list(self.designObjects.values()):
                    obj.CleanUp()
    ...
    def DeletePort(self, port):
        ...
        if port.GetParentOp() == self:
            ...
            if ptype & MAT:
                if ptype & IN:
                    for k, v in list(self.ports_mat_IN.items()):
                        if v == port:
                            portName = k
                            break
                elif ptype & OUT:
                    for k, v in list(self.ports_mat_OUT.items()):
                        if v == port:
                            portName = k
                            break
                ...
                if ptype & IN:
                    for k, v in list(self.ports_ene_IN.items()):
                        if v == port:
                            portName = k
                            break
                elif ptype & OUT:
                    for k, v in list(self.ports_ene_OUT.items()):
                        if v == port:
                            portName = k
                            break

            elif ptype & SIG:
                for k, v in list(self.ports_sig.items()):
                    if v == port:
                        portName = k
                        break
    ...
    def GetPorts(self, portType=IN | OUT | MAT | ENE | SIG):
        ...
        if portType & MAT:
            if portType & IN:
                ports.extend(list(self.ports_mat_IN.values()))
            if portType & OUT:
                ports.extend(list(self.ports_mat_OUT.values()))
        if portType & ENE:
            if portType & IN:
                ports.extend(list(self.ports_ene_IN.values()))
            if portType & OUT:
                ports.extend(list(self.ports_ene_OUT.values()))
        if portType & SIG:
            ports.extend(list(self.ports_sig.values()))
        return ports
    ...
    def GetPortNames(self, portType=IN | OUT | MAT | ENE | SIG):
        ...
        if portType & MAT:
            if portType & IN:
                names.extend(list(self.ports_mat_IN.keys()))
            if portType & OUT:
                names.extend(list(self.ports_mat_OUT.keys()))
        if portType & ENE:
            if portType & IN:
                names.extend(list(self.ports_ene_IN.keys()))
            if portType & OUT:
                names.extend(list(self.ports_ene_OUT.keys()))
        if portType & SIG:
            names.extend(list(self.ports_sig.keys()))
        return names
    ...
    def GetPortItems(self, portType=IN | OUT | MAT | ENE | SIG):
        ...
        if portType & MAT:
            if portType & IN:
                items.extend(list(self.ports_mat_IN.items()))
            if portType & OUT:
                items.extend(list(self.ports_mat_OUT.items()))
        if portType & ENE:
            if portType & IN:
                items.extend(list(self.ports_ene_IN.items()))
            if portType & OUT:
                items.extend(list(self.ports_ene_OUT.items()))
        if portType & SIG:
            items.extend(list(self.ports_sig.items()))
        return items
    ...
    def GetAllChildConnections(self, portType=IN | OUT | MAT | ENE | SIG):
       ...
       Order of info -- (uOpNameOut, portNameOut, uOpNameIn, portNameIn)
        """
        conns = []
        for uOp in list(self.chUODict.values()):
    ...
    def ShortestPortPath(self, port):
        ...
        def GetPathOfPortFrom(obj, given_port, portDict):
            """
            return path of port in portDict or None if not there
            """
            try:
                i = list(portDict.values()).index(given_port)
                return obj.GetPath() + '.' + list(portDict.keys())[i]
            except ValueError:
                return None
    ...
    def ShareProperties(self, v, p1, p2):
        ...
        if not isinstance(v, str):  # if v isn't string then should be sequence
            for i in v:
    ...
    def GetChildUONames(self):
        ...
        return list(self.chUODict.keys())
    ...
    def GetChildUnitOps(self):
        ...
        return list(self.chUODict.items())
    ...
    def _GetPreviousCompoundNames(self):
        ...
        if hasattr(self, "_tempCmpNames"):
            if self._tempCmpNames is not None:
                return self._tempCmpNames
        if self.parentUO is not None:
    ...
    def _GetCompoundsMap(self):
        ...
        if hasattr(self, "_tempMapCmps"):
            if self._tempMapCmps is not None:
    ...
    def ThermoChanged(self, thCaseObj):
        ...
        if oldCmpNames != newCmpNames:
            if oldCmpNames is not None and newCompoundCount > 0:
        ...
        for port in self.GetPorts(MAT | IN | OUT):
            ...
            if oldValues is not None and doMapping and None not in oldValues:
                setValues = True
                status = np.zeros(newCompoundCount, dtype=int)
                ...
                if setValues:
                    ...
                    if newValues is not None and None not in newValues:
        ...
        for op in list(self.chUODict.values()):
            try:
                if op.thCaseObj is None:
    ...
    def SetParameterValue(self, paramName, value):
        ...
        if paramName not in PARAMS_DONOTTRIGGER_SOLVE:
            self.ForgetAllCalculations()
    ...
    def ParameterChanged(self, paramName, value):
        ...
        for uo in list(self.chUODict.values()):
            if paramName not in uo.parameters:
                try:
                    uo.ParameterChanged(paramName, value)
                except:
                    self.InfoMessage('ErrNotifyParChange', (uo.GetPath(), paramName, value),
    ...
    def GetParameterValue(self, paramName):
        ...
        if result is None and self.parentUO:
    ...
    def GetTolerance(self):
        if MAXERROR_PAR in self.parameters:
            ...
            return self.parentUO.GetTolerance()
        else:
            return 0.0001
    ...
    def PopSolveOp(self):
        ...
        if self.parentUO:
            return self.parentUO.PopSolveOp()
        else:
            return None
    ...
    def PopForgetOp(self):
        ...
        if self.parentUO:
            return self.parentUO.PopForgetOp()
        else:
            return None
    ...
    def PopResetCalcPort(self):
        ...
        if self.parentUO:
            return self.parentUO.PopResetCalcPort()
        else:
            return None
    ...
    def PopResetFixedPort(self):
        ...
        if self.parentUO:
            return self.parentUO.PopResetFixedPort()
        else:
            return None
    ...
    def PopIterationProperty(self):
        ...
        if self.parentUO:
            return self.parentUO.PopIterationProperty()
        else:
            return None
    ...
    def PopConsistencyError(self):
        ...
        if self.parentUO:
            return self.parentUO.PopConsistencyError()
        else:
            return None
    ...
    def GetDesignObjects(self):
        ...
        return list(self.designObjects.values())

    def GetDesignItems(self):
        """Get tuples of (name, obj) of the design object"""
        return list(self.designObjects.items())
    ...
    def CloneContents(self, clone):
        ...
        attrNamesToClone = list(self.__dict__.keys())
    ...
    def _CloneThermo(self, clone, attrNamesToClone):
        ...
        if thermoAdmin is not None:
            thermoAdmin.LinkUO(clone)
        if thCaseObj is not None:
    ...
    def _CloneChildrenOps(self, clone, attrNamesToClone):
        ...
        for childName in self.chUODict:
            ...
            if clone.chUODict.get(childName, None) is not None:
                # Already there, skip
                child.CloneContents(clone.chUODict.get(childName, None))
            else:
                childClone = child.Clone()
                if childClone is not None:
            ...
            for portName in myPorts:
                ...
                if clonePort is not None:
                    ...
                    if self is portParent and clone is clonePortParent:
                        ...
                        if clonePortParent is not match:
                            # Delete the port and clone it from scratch
                            clone.DeletePortNamed(portName)

                if clonedPorts.get(portName, None) is None:
                    portParent = port.GetParent()
    ...
    def _CloneRemainingAttr(self, clone, attrNamesToClone):
        ...
        while attrName:
            ...
            if isinstance(attr, Ports.Port):
                ...
                if attrClone is None and currentClone is not None:
...
class MapCompounds(object):
    ...
    def FromOldToNew(self, oldVals, missingVal=None):
        ...
        for newIdx in range(len(newCmps)):
            ...
            if oldIdx is None or oldIdx == []:
                newVals.append(missingVal)
            elif isinstance(oldIdx, list):
                val = None
                for idx in oldIdx:
                    if oldVals[idx] is not None:
                        if val is None:
                            val = oldVals[idx]
                        else:
                            val += oldVals[idx]
    ...
    def DelRuleForOld(self, oldCmpName):
        if oldCmpName in self.rulesNewToOld:
    ...
    def DelRuleForNew(self, newCmpName):
        if newCmpName in self.rulesOldToNew:
...
def _SafeClone(item):
    ...
    if isinstance(item, float):
        ...
        clone = {}
        for key, content in list(item.items()):
            clone[_SafeClone(key)] = _SafeClone(content)
        return clone
    elif type(np.array([1], dtype=float)) == type(item) and hasattr(item, 'copy'):
        return item.copy()
    else:
        return None
...
def CalculateNonSupportedFlash(unitOp, frac, knownTargetProp, knownFlashProp, iterProp, lastSoln=None, min_val=None,
    ...
    try:
        ...
        lstIterProp[1] = min_val
        while lstIterProp[1] < max_val:
            lstIterProp[1] += step
            try:
                vals = thAdmin.GetProperties(prov, case, knownFlashProp, lstIterProp, OVERALL_PHASE, frac, propList)
                couldInit = 1
    ...
    while iteration < maxIter:
        ...
        while 1:
            ...
            try:
                newVals = thAdmin.GetProperties(prov, case, knownFlashProp, newLstIterProp, OVERALL_PHASE, frac,
                                                propList)
                newError = (newVals[0] - target) / scaleFactor
